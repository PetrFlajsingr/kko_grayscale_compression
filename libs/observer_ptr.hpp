...
//
// std::experimental::observer_ptr<> is a C++98 onward implementation for std::observer_ptr as of C++17.
//
...
#define nsop_REQUIRES_T(VA) \
    , typename = typename std::enable_if< (VA), std::experimental::observer_ptr_lite::detail::enabler >::type
...
{
    size_t operator()(::std::experimental::observer_ptr<T> p ) const nsop_noexcept
    {
        return hash<T*>()( p.get() );
    }
};
